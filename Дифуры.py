# -*- coding: utf-8 -*-
"""
Created on Thu Sep  9 00:20:12 2021

@author: Колесников Александр
"""
...
C0 = solve(eq)[0] # нашли С
dsol0 = dsol.subs(C, C0) 
print('2.',dsol0, solve(dsol0-y0,x)[0]==x0)
...
C0 = solve(eq)[0] # нашли С
dsol0 = dsol.subs(C, C0) 
print('4.',dsol0, solve(dsol0-y0,x)[0]==x0)
eq = Eq(dydx+x**3,1)
sol = solve(eq, dydx)[0]
dsol = integrate(sol, x) + C
x0, y0 = -1, 2 # см. условие задачи выше
...
C0 = solve(eq)[0] # нашли С
dsol0 = dsol.subs(C, C0) 
print('6.',dsol0, solve(dsol0-y0,x)[0]==x0)
eq = Eq(dydx*x**2, 1-x)
sol = solve(eq, dydx)[0]
dsol = integrate(sol, x) + C
